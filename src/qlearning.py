...
def report_sarsa(stochastic):
    if stochastic:
        param_ = {'episodes': [1000, 5000], 'epsilon_decay':[0.01, 0.001], 'alpha':[0.001, 0.01, 0.1], 'gamma':[1.0, 0.99, 0.9]}
    else:
        param_ = {'episodes': [1000, 5000], 'epsilon_decay':[0.01, 0.001], 'alpha': [0.001, 0.01, 0.1], 'gamma': [1.0, 0.99, 0.9]}
...
if __name__ == '__main__':
    ...
    #LEARN
    Q_table, stats = qlearning_tabular(env, episodes=10000, epsilon_max=1.0, epsilon_decay=0.001, min_epsilon=0.01, gamma_=0.99, alpha=0.1 )

    print_policy_sarsa(Q_table)
    #print_state_values_sarsa(Q_table)

    qlearning_tabular_test(env, Q_table, episodes=1, display=False)
    print_policy_sarsa(Q_table)
    """

    report = report_sarsa(True)
    print(report)
